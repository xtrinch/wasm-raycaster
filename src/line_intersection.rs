...
impl<T: Float + std::fmt::Debug> LineRelation<T> {
    pub fn unique_intersection(self) -> Option<Coord<T>> {
        match self {
...
impl<T: Float + std::fmt::Debug> LineInterval<T> {
    ...
    fn cross(a: &Coord<T>, b: &Coord<T>) -> T {
        a.x * b.y - a.y * b.x
    }

    fn div(a: &Coord<T>, b: T) -> Coord<T> {
        (a.x / b, a.y / b).into()
    }

    fn t_coord_to_point(p: &Coord<T>, r: &Coord<T>, t: T) -> Coord<T> {
